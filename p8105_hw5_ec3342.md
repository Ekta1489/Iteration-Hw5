...
\#Making a spaghetti plot showing observations on each subject over
time, to find the difference between the two groups.
...
\#As we can see from the two plots above, in the experimental arm there
is an increase in the observation score with time. In the control arm,
...
in the observation scores with time. However, further analysis should be
done to conclude this.
